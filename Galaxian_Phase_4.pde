...
Sprite ship, monster, falling, rocket, explosionP, font;
...
int gameMode = 2; // 0 = Main Menu (not yet programmed), 1 = Endless, 2 = Boss
...
- monster / grid[index] (an array of weak monsters that the player faces)
...
//Initiates basic falling monster
...
void changeFalling()
{
  ...
  if (!fallingA.isDead())
  {
    ...
    if (fallingA.getX() >= width || fallingA.getX() <= 0)
      fmA_Speed = -fmA_Speed;
...
void drawGameOver()
{
  if(gameOver == true)
  {
    font = new Sprite(this, "gameOver.jpg", 1, 1, 40);
    font.setXY(width/2, height - 275);
    font.setScale(1.2);
  }
}

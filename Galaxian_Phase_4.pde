...
//instantiating a sprite array/grid for the monsters, explosions and missiles
Sprite [] grid;
Sprite [] explosion;
Sprite [] shotC;
Sprite [] fireball;
...
public void setup()
{
  ...
  // Creates arrays for endless and boss fight
  grid = new Sprite[50];
  shotC = new Sprite[2];
...
void pre()
{
  ...
  // Checks if an object has exploded
  checkExplode();
...
Contains the build function for every object used in the game. 
*******************************************************************************************/
...
//Builds the different Falling Monsters
void buildFalling()
{
...
// Builds the boss and its phases
void buildBoss()
{
...
void changeFalling()
{
  ...
  if (!fallingC.isDead())
  {
    ...
    //Move falling monster C downwards and sideways as needed
...
//Draws Game Over Screen or Congratulations Screen after boss is defeated
void drawGameOver()
{
...
//Draws Score (both during game and after game is over)
void drawscore()
{
...
// Resets the screen between phases
void reset()
{
  gameMode = 0;
  ship.setDead(true);
